...
class PaykunProcessor extends \Magento\Framework\App\Action\Action {
            ...
            if($orderId) {
                ...
                if(in_array($this->_orderDetail->getOrderCurrencyCode(), self::ALLOWED_CURRENCIES)) {
                    ...
                    if($formData !== null && $this->_isError !== true) {
                        ...
                        $this->_logger->addInfo('Start Payment for Merchant Id => '.
                    ...
                    $this->_logger->addError('Failed => ['.$this->_errorCode.'] => '. $this->_errorMessage);
                ...
                $this->_logger->addError('Failed => ['.ErrorCodes::SESSION_ORDER_ID_NOT_FOUND_STRING.'] => '. $this->_errorMessage);
    ...
    private function restoreCart($errorMsg) {
        ...
        if ($this->_checkoutSession->restoreQuote()) {
            ...
            $this->_logger->addInfo("Cart restored success fully.");
            ...
            $this->_logger->addError("PAYKUN Couldn't restored your cart please try again.");
            $this->_redirect('checkout', ['_fragment' => 'payment']);
    ...
    private function getPaykunPaymentDetail($orderId) {
        ...
        try {
            ...
            $this->_logger->addError('Failed => ['.
                $this->_errorCode.'] => '. $this->_errorMessage);
    ...
    private function getRealOrderId() {
        ...
        try {
            ...
            $orderId = $this->_checkoutSession->getLastRealOrder();
            return $orderId;
